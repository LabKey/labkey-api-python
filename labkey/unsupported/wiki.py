...
def update_wiki(server_context, wiki_name, wiki_body, container_path=None):
    ...
    try:
        read_response = server_context.make_request(
            read_wiki_url,
            payload,
            headers=headers,
            method="GET",
        ...
        return (
            1  # TODO: this is incorrect, should return 'success'/'error' properly like the docs say
        )
    ...
    if v is None:
        v = next(
            (i for i in range(len(data_list)) if "LABKEY._wiki.setProps" in data_list[i]),
            None,
        )
    ...
    update_wiki_url = server_context.build_url(
        "wiki", "saveWiki.api", container_path=container_path
    )
    headers = {"Content-type": "application/json"}
    ...
    try:
        data = server_context.make_request(
            update_wiki_url,
            payload=json_dumps(wiki_vars, sort_keys=True),
            headers=headers,
            non_json_response=True,
        )
    except SSLError as e:
